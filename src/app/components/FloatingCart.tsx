...
type FlowingCartProps = {
  count: number;
  items: number;
  images: string[];
};

const FlowingCart = ({ count, items, images }: FlowingCartProps) => {
  return (
    <Link
      className="flex flex-col items-center w-screen h-auto bg-[url(/floatingCart2.svg)] bg-center bg-cover bg-no-repeat group cursor-pointer"
      href={`/cart`}
    >
      {/* Top drag indicator */}
      ...
      <div className="flex items-center justify-between  rounded-full px-2 py-5 w-full">
        ...
        <div className="flex items-center gap-4 w-[40%]">
          ...
          <div className="leading-tight">
            ...
            <p className="text-sm text-black/80">{items} items</p>
          </div>
        </div>
